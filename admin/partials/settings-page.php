...
<div class="wrap">
    ...
    <form method="post" action="options.php" class="wp-map-settings-form">
        ...
        <div class="wp-map-settings-grid">
            <div class="wp-map-settings-column">
                <!-- 第一列：基础设置 -->
                ...
                <div class="wp-map-form-group">
                    <label for="map_path_enabled"><?php _e('地图路径功能', 'wp-map'); ?></label>
                    <div class="wp-map-checkbox-group">
                        <label>
                            <input type="checkbox" id="map_path_enabled" name="wp_map_settings[map_path_enabled]" value="yes" <?php checked(isset($settings['map_path_enabled']) ? $settings['map_path_enabled'] : 'no', 'yes'); ?>>
                            <?php _e('启用地图路径功能', 'wp-map'); ?>
                        </label>
                    </div>
                    <p class="description"><?php _e('启用后可以访问前端地图页面', 'wp-map'); ?></p>
                    
                    <div id="map_path_settings" style="margin-top: 10px; <?php echo (isset($settings['map_path_enabled']) && $settings['map_path_enabled'] === 'yes') ? '' : 'display: none;'; ?>">
                ...
                <div class="wp-map-form-group">
                    <label for="shortcode_enabled"><?php _e('短代码功能', 'wp-map'); ?></label>
                    <div class="wp-map-checkbox-group">
                        <label>
                            <input type="checkbox" id="shortcode_enabled" name="wp_map_settings[shortcode_enabled]" value="yes" <?php checked(isset($settings['shortcode_enabled']) ? $settings['shortcode_enabled'] : 'no', 'yes'); ?>>
                            <?php _e('启用短代码功能', 'wp-map'); ?>
                        </label>
                    </div>
                    <p class="description"><?php _e('启用后可以在文章和页面中使用短代码显示地图', 'wp-map'); ?></p>
                    <?php if (isset($settings['shortcode_enabled']) && $settings['shortcode_enabled'] === 'yes') : ?>
                    <div class="wp-map-shortcode-info">
                        <p><strong><?php _e('短代码示例：', 'wp-map'); ?></strong></p>
                        <code style="display: block; background: #f5f5f5; padding: 10px; margin: 5px 0; border-radius: 3px;">[wp_map height="500px"]</code>
                        <p class="description">
                            <?php _e('参数说明：', 'wp-map'); ?><br>
                            <?php _e('<code>height</code> - 地图高度（默认：500px）', 'wp-map'); ?>
                        </p>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="wp-map-settings-column">
                ...
                <div class="wp-map-form-group">
                    ...
                    <p class="description"><?php _e('选择地图的显示样式，设置后会在前端地图中生效', 'wp-map'); ?></p>
                </div>
            </div>

            <div class="wp-map-settings-actions">
...
jQuery(document).ready(function($) {
    ...
    // 地图路径功能切换显示
    $('#map_path_enabled').change(function() {
        if ($(this).is(':checked')) {
            $('#map_path_settings').slideDown();
        } else {
            $('#map_path_settings').slideUp();
        }
    });
});
</script>